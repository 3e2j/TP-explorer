...
fn read_u32_at_reads_big_endian_header_values() {
    ...
    let result = {
        ...
        iso_read::read_u32_at(&mut file, 0x428).expect("read")
    };
    assert_eq!(result, 0);
}
...
fn parse_iso_files_extracts_one_file() {
    ...
    assert_eq!(
        iso_read::parse_iso_files(&iso_path).unwrap()[0].path,
        "files/a.txt"
    );
}
...
fn read_iso_file_bytes_returns_the_expected_payload() {
    let iso_path = common::build_single_file_iso("a.txt", b"DATA");
    assert_eq!(
        iso_read::read_iso_file_bytes(&iso_path, "a.txt").unwrap(),
        b"DATA"
    );
}
...
fn build_iso_hash_map_uses_relative_paths() {
    let iso_path = common::build_single_file_iso("a.txt", b"DATA");
    assert!(iso_read::build_iso_hash_map(&iso_path)
        .unwrap()
        .contains_key("a.txt"));
}
...
fn rebuild_iso_with_files_applies_replacements() {
    ...
    let files = iso_read::parse_iso_files(&source).unwrap();
